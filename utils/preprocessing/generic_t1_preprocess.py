...
if __name__ == "__main__":
    ...
    parser.add_argument('--mni_template_img_path', '-mni', type=str, default='/usr/local/fsl/data/standard/MNI152_T1_1mm_brain.nii.gz', help='MNI template image path', required=False)

    args = parser.parse_args()
    output_dir = args.output_dir
    input_images = args.input_images
    mni_template_img_path = args.mni_template_img_path

    # Define the normalization method
    normalize_method = FCMNormalize
    for input_image in input_images:
        cropped_path = process_niftis(input_image, output_dir, normalize_method, normalize_prefix="fcm_", mni_template_img_path=mni_template_img_path)
