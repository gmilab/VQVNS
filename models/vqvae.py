...
class BrainVQVAE(pl.LightningModule):
    ...
    def get_generator_loss(self, x, x_hat, quant_loss):
        ...
        if self.discriminator is not None and self.current_epoch >= self.adversarial_loss_start_epoch:
            ...
            #epoch_adjusted_adv_weight = self.adv_weight * min(1.0, self.current_epoch / self.adv_epoch_weighting_denominator)
    ...
    def on_validation_epoch_end(self):
        ...
        # #They have to be tensors
        # self.val_xhat_samples = torch.tensor(self.val_xhat_samples).squeeze(dim=0)
        # self.val_x_samples = torch.tensor(self.val_x_samples)

        # #Loss metrics that are *not* part of the total loss
        # mssim = 1 - self.mssim_loss(self.val_xhat_samples, self.val_x_samples)
        # gmi = -self.gmi_loss(self.val_xhat_samples, self.val_x_samples)
        # lncc = 1 - self.lncc_loss(self.val_xhat_samples, self.val_x_samples)

        # self.log('val_mssim', mssim, on_epoch=True, on_step=False)
        # self.log('val_gmi', gmi, on_epoch=True, on_step=False)
        # self.log('val_lncc', lncc, on_epoch=True, on_step=False)
